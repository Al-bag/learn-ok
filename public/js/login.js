document.addEventListener('DOMContentLoaded', () => {
  ...
  // Hashes text using the browser's built-in SubtleCrypto API.
  ...
  async function handleSignup() {
    ...
    const signupData = {
      ...
      username: await sha256(plainUsername), // Hashed username
      password: await sha256(passwordInput.value), // Hashed password
      plainUsername: plainUsername // Send plain username for storage
    };
    ...
    if (res.ok) {
      ...
      const userToStore = {
        username: plainUsername, // Store plain username for identification
  ...
  async function handleLogin() {
    const plainUsername = usernameInput.value;
    const loginData = {
      username: await sha256(plainUsername),
    ...
    if (res.ok) {
      ...
      // The server returns user data without the password.
      // We add the plain username for identification on the client.
      const userToStore = { ...userData, username: plainUsername };
      localStorage.setItem('user', JSON.stringify(userToStore));
      window.location.href = 'chat.html';
    } else {
